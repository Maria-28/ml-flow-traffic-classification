"""
DDoS Detection Research Framework: Boosting vs Baseline Comparison.
Dataset: CIC-IDS2017 (Friday-DDoS)

This module implements a thesis-grade research pipeline for comparing
Gradient Boosting (CatBoost, XGBoost) against classical ML baselines.

Architecture:
    - DDoSExperiment class: encapsulates all experiment state and logic
    - YAML configuration: all parameters externalized, no magic numbers
    - CLI interface: full control over experiment execution
    - Statistical testing: McNemar test + Bootstrap confidence intervals

Standards:
    - PEP8 & Black formatted
    - Type-hinted (mypy-compatible)
    - Reproducible (fixed seeds + environment logging)
    - Resilient (caching, graceful degradation, error recovery)

Usage Examples:
    # Run with default config
    python boosting_ddos.py

    # Custom config, force retrain, 5-fold CV
    python boosting_ddos.py --config my_config.yaml --cv 5 --force-retrain

    # Only boosting models, quiet mode
    python boosting_ddos.py --models boosting --quiet

    # Show help
    python boosting_ddos.py --help
...
class DataConfig:
    ...
    path: str = "data/raw/Friday-DDos.pcap_ISCX.csv"
...
class DDoSExperiment:
    ...
    def prepare_data(self) -> None:
        ...
        try:
            df = load_and_clean_data(data_path, verbose=self.verbose)
